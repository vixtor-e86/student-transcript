...
export default function Student() {
  ...
  const handleSearch = useCallback(
    async (e: React.FormEvent) => {
      ...
      const transcript = findByMatricNumber(matricNumber);

      if (transcript) {
        setResult({ found: true, transcript });
      } else {
        setResult({
          found: false,
          message:
            'No transcript found for this matric number. Please verify your matric number and try again, or contact the records office.',
        });
      }

      setLoading(false);
    },
    [matricNumber, findByMatricNumber]
  );
