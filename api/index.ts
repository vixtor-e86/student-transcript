...
export default async function handler(request: Request) {
  ...
  try {
    ...
    // handleUpload is used for client-side uploads
    if (request.method === 'POST') {
      const url = new URL(request.url);
      const action = url.searchParams.get('action');

      // ACTION 1: Generate Client-Side Upload Token
      if (action === 'upload') {
        ...
        try {
          const jsonResponse = await handleUpload({
            ...
            onUploadCompleted: async ({ blob, tokenPayload }) => {
              // This is called after the file is successfully uploaded to Blob
      ...
      // ACTION 2: Save metadata to KV (called after Blob upload is done)
      ...
      const newTranscript = {
        ...
        fileUrl: fileUrl, // Already uploaded URL
